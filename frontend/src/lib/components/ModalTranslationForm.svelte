<script>
    ...
    const getAvailableLangs = () => {
        const fetchLanguages = () => {
            const translationHost = env.PUBLIC_TRANSLATION_API_HOST || env.PUBLIC_API_HOST || "http://localhost:8082";
            fetch(`${translationHost}/languages`)
            .then(res => res.json())
            .then(data => {
                if (data) {
            ...
            .catch(error => {
                ...
                // Stop trying after errors to avoid spam
                clearInterval(fetchLanguagesInterval);
            });
        };
